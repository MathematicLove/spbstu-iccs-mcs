...
import hashlib
import itertools
from config_DAC import PASSWD_FILE, HASH_ALGO, PASSWORD_ALPHABET, FIRST_CHAR_ALPHABET
...
# passwd format (DAC): login:hash:uid:fullname (elevate/drop to access 0600 root)
def read_passwd():
    users = {}
    elevate()
    try:
...
def main():
    if len(sys.argv) < 2:
        print("use:")
        print("crack_DAC.py LOGIN LENGTH - Permutation")
        print("crack_DAC.py --max-iterations - max iterations for 3-8")
        sys.exit(1)
    ...
    if len(sys.argv) != 3:
        print("use: crack_DAC.py <login> <length>")
        sys.exit(1)
